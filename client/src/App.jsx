...
const App = React.memo(function App() {
  ...
  // Aggressive color fix for monitor compatibility
  useEffect(() => {
    const forceColorFix = () => {
      // Force page background
      document.body.style.setProperty('background-color', '#020817', 'important');
      document.documentElement.style.setProperty('background-color', '#020817', 'important');
      
      // Only target form containers for shadow removal
      const formContainers = document.querySelectorAll('.min-h-screen');
      formContainers.forEach(el => {
        // Only remove shadows from login/register pages
        if (el.classList.contains('items-center') && el.classList.contains('justify-center')) {
          el.style.setProperty('box-shadow', 'none', 'important');
        }
        el.style.setProperty('background-color', '#020817', 'important');
      });
      
      // Force main content areas but skip card elements
      const mainElements = document.querySelectorAll('main, .bg-surface-900');
      mainElements.forEach(el => {
        // Skip navigation elements and card elements
        if (el.closest('header') || el.closest('aside') || el.closest('nav') || 
            el.classList.contains('game-card') || 
            el.classList.contains('bg-surface-800') ||
            el.hasAttribute('data-friend-id')) {
          return;
        }
        el.style.setProperty('background-color', '#020817', 'important');
      });

      // Force card backgrounds to be visible
      const cardElements = document.querySelectorAll('.bg-surface-800, .game-card, [data-friend-id]');
      cardElements.forEach(el => {
        el.style.setProperty('background-color', '#374151', 'important');
        el.style.setProperty('border', '1px solid #4B5563', 'important');
      });

      // Ensure input icons are always visible
      const inputIcons = document.querySelectorAll('.relative span.absolute svg');
      inputIcons.forEach(icon => {
        icon.style.setProperty('display', 'block', 'important');
        icon.style.setProperty('visibility', 'visible', 'important');
        icon.style.setProperty('opacity', '1', 'important');
        icon.style.setProperty('color', '#9CA3AF', 'important');
        icon.style.setProperty('z-index', '10', 'important');
      });
    };
    
    // Run multiple times to override stubborn caching
    forceColorFix();
    const timer1 = setTimeout(forceColorFix, 25);
    const timer2 = setTimeout(forceColorFix, 100);
    const timer3 = setTimeout(forceColorFix, 300);
    const timer4 = setTimeout(forceColorFix, 1000);
  ...
  return (
    <div className="flex min-h-screen bg-surface-900" style={{backgroundColor: '#020817'}}>
      <Header />
      <div className="flex-1 overflow-hidden bg-surface-900" style={{backgroundColor: '#020817'}}>
        <main className="h-screen overflow-y-auto pt-20 md:pl-64 bg-surface-900" style={{backgroundColor: '#020817'}}>
          <Outlet />
        </main>
      </div>
      {/* <Footer /> */}
