...
export default function ScrollableGameCards({ games, type, onToggleExpand, fixedHeight }) {
  ...
  return (
    <div className="relative px-4 sm:px-2">
      <div className="absolute left-6 sm:left-4 top-1/2 -translate-y-1/2 z-10 flex-col items-start hidden sm:flex">
        <button 
          onClick={scrollLeft}
      ...
      <div 
        ...
        className="flex space-x-4 overflow-x-auto py-4 hide-scrollbar snap-x snap-mandatory px-4 sm:px-0"
      ...
      <div className="absolute right-6 sm:right-4 top-1/2 -translate-y-1/2 z-10 flex-col items-end hidden sm:flex">
        <button 
          onClick={scrollRight}
          disabled={isScrolling}
          className="bg-surface-800/80 h-10 w-10 flex items-center justify-center rounded-full shadow-lg hover:bg-primary-600/70 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-600 disabled:opacity-50"
