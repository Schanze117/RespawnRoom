...
const EmptyFriendsList = ({ searchQuery, toggleSearchMode }) => (
  <div className="col-span-full p-10 bg-surface-800 rounded-lg border border-dashed border-surface-700 flex flex-col items-center justify-center">
    <svg xmlns="http://www.w3.org/2000/svg" className="w-16 h-16 text-gray-500 mb-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
    ...
    {searchQuery ? (
      <>
        <p className="text-white font-medium text-lg mb-2">No friends found matching "{searchQuery}"</p>
        <p className="text-gray-400 text-center mb-4">Try a different username or check your spelling</p>
      </>
    ) : (
      <>
        <p className="text-white font-medium text-lg mb-2">Your friends list is empty</p>
        <p className="text-gray-400 text-center mb-6">Add friends to see them here</p>
        <button 
          className="px-6 py-3 bg-primary-600 text-white rounded-lg font-medium hover:bg-primary-700 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center gap-2"
